...
- [AI Model Helper](implement-ai-helper.md) — floating chat panel with prompt + image + PDF input; LLM emits `ModelFileV1` JSON validated by existing `ModelFileV1Schema` and applied via existing `applyToStore`; pluggable providers (GitHub Models default + Anthropic / OpenAI / Custom OpenAI-compatible / Manual paste); per-provider security warnings inline in settings dialog; repair loop on Zod failure; lazy-loaded `pdfjs-dist`. No schema bump.
