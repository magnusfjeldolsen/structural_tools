 ...
 * Uses pure Python implementation with Pyodide (no structuralcodes dependency)
...
async function initializePyodide() {
    ...
    try {
        ...
        // Load pure Python concrete bending module
        statusEl.textContent = 'Loading EC2 analysis module...';
        progressEl.style.width = '85%';

        // Inline the Python code directly to avoid CORS issues with file://
        await appState.pyodide.runPythonAsync(await (await fetch('concrete_bending_pure.py')).text());
        ...
        console.log('Pyodide loaded successfully with pure Python EC2 implementation');
...
function drawBasicOutline() {
    ...
    // Add rebars to visualization
    appState.sectionParams.reinforcements.forEach((r, idx) => {
        if (r.type === 'single_bar') {
            vizData.rebars.push({
                x: r.x,
                y: r.y,
            ...
            // Estimate rebar positions for visualization
            const boundary = r.boundary;
            const spacing = r.spacing;
            const boundaryCover = r.boundary_cover;
            const startCover = r.start_cover || boundaryCover;

            if (boundary === 'bottom') {
                const y = -height/2 + boundaryCover;
                const x_start = -width/2 + startCover;
                const x_end = width/2 - (r.end_cover || boundaryCover);
                const length = x_end - x_start;
                const n_bars = Math.floor(length / spacing) + 1;
...
import json
from concrete_bending_pure import run_analysis
