...
Pull requests into `master` must pass one **required status check** — `2dfea type-check + build + test`, produced by `.github/workflows/pr-checks.yml`. It is enforced by a GitHub **repository ruleset**, which means:

- The merge button (and `gh pr merge`) stays **blocked until the check is green**, and it is **not bypassable with `--admin`** (rulesets are stricter than classic branch protection).
- The check **always runs** on every PR so it always reports, but a `git diff` step **skips the heavy 2dfea build/type-check/test when no 2dfea-relevant files changed** — HTML-only PRs pass in seconds. It runs the full 2dfea build when a PR touches `2dfea/**`, `pr-checks.yml`, or `deploy-all-modules.yml`.
  - ⚠️ Do **not** re-add a top-level `paths:` filter to `pr-checks.yml`. A path-filtered required check never reports on PRs outside those paths, which leaves every non-2dfea PR **permanently un-mergeable**.

### Serving & merge mechanics

- GitHub Pages serves the **`gh-pages`** branch in **legacy "deploy from a branch"** mode. `gh-pages` is generated by the deploy workflow (`peaceiris/actions-gh-pages`, `keep_files: false` = atomic wipe-and-republish) — **never hand-edit `gh-pages`**; the source of truth is `master`.
- PRs are **rebase-merged**. After a rebase-merge, `gh pr merge --delete-branch` frequently **cannot fast-forward your local `master`** (it diverges from `origin/master`), and the working tree may briefly show pre-merge content. This is expected — trust `origin/master` and the live site (`git grep <marker> origin/gh-pages` to verify what's deployed), and run `git reset --hard origin/master` to resync local `master`.
...
**2. `module-registry/module-registry.json` — do NOT hand-edit it.** It is **auto-generated on every deploy** by `npm run update-registry` (`module-registry/generate-registry.js`); any manual edits are overwritten. The generator scans each top-level folder for `index.html` and reads its `<title>`, `<meta name="description">`, and `<meta name="keywords">`. So instead of editing the JSON:

- Ensure the module has **`<module>/index.html`** with a good `<title>` + description/keywords meta. The registry `url` is hardcoded to `./<folder>/index.html`, so a module whose entry file is named anything else (e.g. `tip-bearing-on-rock.html`) **will not be discovered** — give it an `index.html` (a landing/redirect page is fine).
- `category` is derived by `determineCategory()` from the folder name and keywords (concrete / steel / timber / geotechnical-by-name / structural-analysis, else `other`).
- `_meta.total_modules` is recomputed automatically. Run `npm run update-registry` locally if you want the committed JSON to match what the deploy will produce.
