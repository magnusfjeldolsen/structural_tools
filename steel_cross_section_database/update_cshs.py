...
# Find the header row (row 4 contains the units)
# Row 2 contains the field names
header_row = 2
unit_row = 4
...
# Get column names from row 3 (symbolic names like b, t, ro, etc.)
...
# Filter only SHS profiles (CSHS)
cshs_data = df_data[df_data['Profile'].notna() & df_data['Profile'].astype(str).str.contains('SHS', na=False)]
...
if os.path.exists(json_file):
    with open(json_file, 'r', encoding='utf-8') as f:
        cshs_json = json.load(f)
else:
    # Create new structure based on sheet type
    profile_type = "HSHS" if sheet_name == "HSHS" else "CSHS"
    description = "Hot formed square hollow sections" if sheet_name == "HSHS" else "Cold formed square hollow sections"
    cshs_json = {
        "metadata": {
...
existing_profiles = {p['profile'] for p in cshs_json['profiles']}

# Convert data to JSON structure
new_profiles = []

for _, row in cshs_data.iterrows():
...
cshs_json['profiles'].extend(new_profiles)

# Sort profiles by size
def parse_profile_name(name):
    # Extract size from profile name like "SHS 20 / 2" or "SHS20x2"
    import re
    match = re.search(r'(\d+)[x/\s]+(\d+\.?\d*)', name)
    if match:
        return (int(match.group(1)), float(match.group(2)))
    return (0, 0)

cshs_json['profiles'].sort(key=lambda x: parse_profile_name(x['profile']))

# Write updated JSON
output_file = sheet_name.lower() + '.json'
with open(output_file, 'w', encoding='utf-8') as f:
    json.dump(cshs_json, f, indent=2, ensure_ascii=False)

print(f"Added {len(new_profiles)} new profiles to {output_file}")
print(f"Total profiles: {len(cshs_json['profiles'])}")